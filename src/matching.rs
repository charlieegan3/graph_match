...
pub fn recusive_node_match(
    query_root_index: usize, graph_root_index: usize, query: &graph::Graph, graph: &graph::Graph, source_edge: Option<edge::Index>)
    -> MatchedComponents {
        if !query.nodes[query_root_index].matches(&graph.nodes[graph_root_index]) {
            return MatchedComponents { list: vec![] }
        }

        let mut matched_components = MatchedComponents { list: vec![Component { from_edge: source_edge, node: graph_root_index }] };
            ...
            match matching_edge_in_graph {
                Some(edge) => {
                    match query.nodes[query.edges[query_edge_index].target].matches(&graph.nodes[graph.edges[edge].target]) {
                        true => {
                            let descendents = recusive_node_match(
